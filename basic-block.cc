...
Eval_Result & Basic_Block::evaluate(Local_Environment & eval_env, ostream & file_buffer)
{
	...
	for (i = statement_list.begin(); i != statement_list.end(); i++)
	{
		...
		if(result->get_result_enum() == skip_result || result->get_result_enum() == goto_result || result->get_result_enum() == skip_result_val || result->get_result_enum() == skip_result_float){
